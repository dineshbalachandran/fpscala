...
object Par {
  ...
  def sumlike[A](s: IndexedSeq[A])(z: A)(f: (A, A) => A): Par[A] = {
    if (s.length <= 1)
      Par.unit(f(s.headOption getOrElse z, z))
    else {
      val (l, r) = s.splitAt(s.length/2)
      map2(fork(sumlike(l)(z)(f)), fork(sumlike(r)(z)(f)))(f(_,_))
    }
  }
}
...
object ParTest {
  def main(args: Array[String]) : Unit = {
    ...
    println(Par.run(S)(Par.sumlike(IndexedSeq(1,2,3,4))(0)(_+_)).get())
    S.shutdown()
  }
}
